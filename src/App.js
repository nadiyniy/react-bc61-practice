...
const App = () => {
	...
	const getFilterData = () => {
		return employees
			...
			.filter((user) => (department === 'all' ? user : user.position.toLowerCase().includes(department.toLowerCase())));
	};
	...
	return (
		<div>
			...
			{isLogin ? <button onClick={logout}>exit</button> : <button onClick={() => login('Andre')}>enter</button>}
